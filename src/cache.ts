...
const saveCache = (cachePath: string, cacheKey: string) => {
  cache.saveCache([cachePath], cacheKey, undefined, false)
    .then((cacheId) => {
...
const restoreCache = (cachePath: string, cacheKey: string) => {
  return cache.restoreCache([cachePath], cacheKey, undefined, undefined, false)
    .then((key) => {
      if (key) {
...
const generateKey = (prefix: string) => {
  ...
  if (options.environmentFile) {
    return fs.readFile(options.environmentFile, 'utf-8')
...
export const saveCacheEnvironment = (environmentName: string) => {
  ...
  return generateKey(options.cacheEnvironmentKey)
